...
FROM
    ...
    LEFT JOIN dwh_peternakan_sapi_wagyu.dimensi_sapi AS dim ON src.id_sapi_wagyu = dim.id_sapi_wagyu
    AND dim.current_flag = 'Y'
    LEFT JOIN dbt_peternakan_sapi_wagyu.tb_produk_daging AS src2 ON src2.id_sapi_wagyu = dim.id_sapi_wagyu
WHERE
    dim.id_sapi_wagyu IS NULL;
...
FROM
    ...
    LEFT JOIN dbt_peternakan_sapi_wagyu.tb_produk_daging AS src2 ON src2.id_sapi_wagyu = dim.id_sapi_wagyu
WHERE
    IFNULL(dim.id_sapi_wagyu, 0) <> IFNULL(src.id_sapi_wagyu, 0)
    OR IFNULL(dim.nama_sapi_wagyu, '') <> IFNULL(src.nama_sapi_wagyu, '')
